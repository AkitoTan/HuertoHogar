...
const logoSrc = "/img/logo.png"; // debe estar en public/img/logo.png
const recomendados = [
...
const Home = ({ user }) => (
  <div style={{
    maxWidth: 1100, margin: "40px auto", padding: 30, background: "#fff", borderRadius: 14, boxShadow: "0 2px 8px #eef", fontFamily: "Montserrat, sans-serif"
  }}>
    <img src={logoSrc} alt="HuertoHogar Logo" style={{ width: 120, margin: "0 auto", display: "block" }} />
    <h1 style={{ color: "#2E8B57", fontFamily: 'Playfair Display, serif', textAlign: "center" }}>
      Bienvenido a HuertoHogar
    </h1>
    <p style={{ textAlign: "center" }}>Tu tienda online de <b>productos frescos y orgánicos</b> directo del campo a tu puerta. Vive saludable y apoya la agricultura local.</p>
      ...
      <div style={{ display: "flex", gap: 24, justifyContent: "center" }}>
        {recomendados.map(prod => (
          <div key={prod.id} style={{
            background: "#faf9ee", borderRadius: 12, padding: 18, width: 240, boxShadow: "0 1px 7px #ddd",
            display: "flex", flexDirection: "column", alignItems: "center"
          }}>
            <img src={prod.img} alt={prod.name} style={{ width: "92%", height: 90, objectFit: "cover", borderRadius: 10, marginBottom: 10, boxShadow: "0 1px 7px #ccc" }} />
            <h3 style={{ color: "#8B4513", marginBottom: 7 }}>{prod.name}</h3>
            <p style={{ margin: 0 }}>{prod.desc}</p>
            <Link to={`/productos`} style={{
              display: "inline-block", marginTop: 8, background: "#2E8B57", color: "#fff", borderRadius: 5, padding: "4px 12px", textDecoration: "none", fontWeight: "bold"
            }}>
              Ver en catálogo
            </Link>
          </div>
    ...
    <div style={{ marginTop: 46 }}>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", justifyContent: "left" }}>
        {blog.map((post, idx) => (
          <div key={idx} style={{
            background: "#f9f9f6", borderRadius: 9, padding: 14, width: 300, boxShadow: "0 1px 5px #ddd"
          }}>
            <h4 style={{ color: "#2E8B57", marginBottom: 6 }}>{post.titulo}</h4>
            <p style={{ marginBottom: 0 }}>{post.resumen}</p>
            <a href={post.link} style={{
              color: "#8B4513", textDecoration: "underline", fontSize: 15, marginTop: 6, display: "inline-block"
            }}>Leer más</a>
          </div>
        ))}
      </div>
